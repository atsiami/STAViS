function eval_coutrot2(pred_path, annot_base_path, res_path, videonum ,splitnum)

addpath(genpath('./code_forMetrics'));
i= str2num(videonum);
split = str2num(splitnum);
annot_path = [annot_base_path '/annotations/Coutrot_db2/'];
%
pred_path = [pred_path '/split' num2str(split) '_results/coutrot2'];
%
annot_file = [annot_base_path '/fold_lists/Coutrot_db2_list_test_' num2str(split) '_fps.txt'];
%
res_path = [res_path '/Coutrot2_results_all_split' num2str(split)];
