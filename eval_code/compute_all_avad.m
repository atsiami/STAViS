function compute_all_avad(resultspath, annot_base_path)

display('****** Evaluation metrics for AVAD ******')
i=1;
for split=1:3
    %% read database info
    annot_file = [annot_base_path '/fold_lists/AVAD_list_test_' num2str(split) '_fps.txt'];
    %
    res_path = [resultspath '/avad_results_all_split'  num2str(split)];
    %
...
fid1=fopen([resultspath, '/','final_results_AVAD.txt'],'w');

fprintf(fid1,'****** Evaluation metrics for AVAD ******\n');
