function compute_all_summe(resultspath, annot_base_path)

display('****** Evaluation metrics for SumMe ******')
i=1;
for split=1:3
    %% read database info
    annot_file = [annot_base_path '/fold_lists/SumMe_list_test_' num2str(split) '_fps.txt'];
    %
    res_path = [resultspath '/summe_results_all_split' num2str(split)];
    %
...
fid1=fopen([resultspath, '/', 'final_results_SumMe.txt'],'w');

fprintf(fid1,'****** Evaluation metrics for SumMe ******\n');
