function eval_diem(pred_path, annot_base_path, res_path, videonum ,splitnum)

addpath(genpath('./code_forMetrics'));
i= str2num(videonum);
split = str2num(splitnum);
annot_path = [annot_base_path '/annotations/DIEM/'];
%
pred_path = [pred_path '/split' num2str(split) '_results/diem'];
%
annot_file = [annot_base_path '/fold_lists/DIEM_list_test_fps.txt'];
%
res_path = [res_path '/diem_results_all_split' num2str(split)];
%
mkdir(res_path);
%% read database info
