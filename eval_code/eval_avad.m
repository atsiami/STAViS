function eval_avad(pred_path, annot_base_path, res_path, videonum ,splitnum)

addpath(genpath('./code_forMetrics'));
i= str2num(videonum);
split = str2num(splitnum);
annot_path = [annot_base_path '/annotations/AVAD/'];
%
pred_path = [pred_path '/split' num2str(split) '_results/avad/'];
%
annot_file = [annot_base_path '/fold_lists/AVAD_list_test_' num2str(split) '_fps.txt'];
%
res_path = [res_path '/avad_results_all_split' num2str(split)];
%
