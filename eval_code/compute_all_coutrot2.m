function compute_all_coutrot2(resultspath, annot_base_path)

display('****** Evaluation metrics for Coutrot db2 ******')
i=1;
for split=1:3
    %% read database info
    annot_file = [annot_base_path '/fold_lists/Coutrot_db2_list_test_' num2str(split) '_fps.txt'];
    %
    res_path = [resultspath '/Coutrot2_results_all_split' num2str(split)];
    %
...
fid1=fopen([resultspath, '/', 'final_results_Coutrot2.txt'],'w');

fprintf(fid1,'****** Evaluation metrics for Coutrot db2 ******\n');
