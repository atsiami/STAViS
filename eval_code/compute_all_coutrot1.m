function compute_all_coutrot1(resultspath, annot_base_path)

display('****** Evaluation metrics for Coutrot db1 ******')
i=1;
for split=1:3
    %% read database info
    annot_file = [annot_base_path '/fold_lists/Coutrot_db1_list_test_' num2str(split) '_fps.txt'];
    %
    res_path = [resultspath '/Coutrot1_results_all_split' num2str(split)];
    %
...
fid1=fopen([resultspath, '/', 'final_results_Coutrot1.txt'],'w');

fprintf(fid1,'****** Evaluation metrics for Coutrot db1 ******\n');
